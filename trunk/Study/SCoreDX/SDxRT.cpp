...
namespace DX
{
		...
		FLOAT MinDepth, FLOAT MaxDepth)
	{
		...
		D3DXMatrixPerspectiveFovLH(&m_matProj, (float)D3DX_PI * 0.25f, Width / Height, 0.1, 1000.0f);
	}
	HRESULT SDxRT::CreateCubeMap(ID3D11Device* pd3dDevice, FLOAT fWidth, FLOAT fHeight)
	{
		HRESULT hr;
	...
	bool SDxRT::Begin(ID3D11DeviceContext* pContext, D3DXVECTOR4 vColor,
	...
	bool SDxRT::Clear(ID3D11DeviceContext* pContext, D3DXVECTOR4 vColor,
		bool bTargetClear, bool bDepthClear, bool bStencilClear)
	{
		if (bTargetClear)
		{
	...
	HRESULT SDxRT::SaveBackBuffer(ID3D11DeviceContext* pContext, T_STR name)
	{
		HRESULT hr = S_OK;
