...
void MMap::CreateVertexData()
{
	...
	for (int iRow = 0; iRow < m_iNumRows; iRow++)
	{
		for (int iCol = 0; iCol < m_iNumCols; iCol++)
		{
			...
			m_pVertexList[iIndex].p = D3DXVECTOR3((iCol - fHalfCols) * m_fCellDistance, 0,//cos(iCol*0.1f) + sin(iRow*0.1f),
				-(iRow - fHalfRows) * m_fCellDistance);
			m_pVertexList[iIndex].n = D3DXVECTOR3(0, 1.0f, 0);
			m_pVertexList[iIndex].c = D3DXVECTOR4(1, 1.0f, 1, 1);
			m_pVertexList[iIndex].t = D3DXVECTOR2((float)iCol / (m_iNumCols - 1),
