...
void MBoxObject::Update(D3DXVECTOR3 vDir, float fDistance)
{
	m_fYaw = vDir.y;
	m_fPitch = vDir.x;
	m_fRoll = vDir.z;
...
void MBoxObject::MoveLook(float fValue)
{
	m_vPos += m_vLook * fValue;
}
void MBoxObject::MoveSide(float fValue)
{
	m_vPos += m_vRight * fValue;
}
void MBoxObject::MoveUp(float fValue)
{
	m_vPos += m_vUp * fValue;
}