...
MBASIS_INPUT_MAP g_InputData;

bool   MInput::InitDirectInput()
{
	HRESULT hr = S_OK;
...
//   or     and      xor           xand          not
//   |       &       ^             없음          !   => 비트연산자
BYTE MInput::KeyCheck(BYTE dwKey)
{
	// GetKeyState()		// 동기적
...
bool		MInput::Frame()
{
	...
	ZeroMemory(&g_InputData, sizeof(MBASIS_INPUT_MAP));
	...
	if (KeyCheck(DIK_F12) == KEY_HOLD)	g_InputData.bChangeCameraType = true;
	if (KeyCheck(DIK_F5) == KEY_HOLD)
	{
		g_InputData.bChangeFillMode = true;
	}
	if (KeyCheck(DIK_F6) == KEY_HOLD)	g_InputData.bChangePrimitive = true;
	if (KeyCheck(DIK_F7) == KEY_HOLD)	g_InputData.bChangeCullMode = true;
	if (KeyCheck(DIK_F8) == KEY_HOLD)	g_InputData.bChangeCameraType = true;
